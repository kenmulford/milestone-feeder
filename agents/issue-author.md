...
- **The invariants binding THIS candidate** (`agents/architect.md` `## Structured return block`): the architect-resolved `INVARIANTS` entries whose `applies_to` names this candidate's tag. Transcribe each entry's `value` into the existing `## Design (recorded, consistent)` block verbatim, literal values intact, and verify the entry's `citation` per the Rigor gate. Never re-derive or override a handed-in `value`: the handed-in text wins byte for byte. When two or more entries bind this candidate, transcribe each `value` independently. A candidate handed no invariant is unaffected: author exactly as you do today.
