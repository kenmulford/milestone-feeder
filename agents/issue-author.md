...
1. **Consistency.** No two recorded design statements contradict each other. Two statements that cannot both be true simultaneously — e.g., "mirror ConfirmImportPage grouping" and "flat list, no collection picker" — are a contract violation. Re-read your Design section in full before returning to confirm it is internally consistent (satisfies `milestone-driver/agents/triage-reviewer.md` criterion 1, Consistency).

2. **Buildability.** Every decision the acceptance criteria require is either *recorded* in the Design section or *resolved* by a STATED convention — a `Convention followed:` line citing your project docs or a sibling `file:line`. Nothing is left for the implementer to invent. A decision with **no conventional default** — an ungroundable product call (what to build, user-facing behavior) — is a PRODUCT gap: you return `STATUS: PRODUCT_GAP`, you do not guess (satisfies `milestone-driver/agents/triage-reviewer.md` criterion 2, Buildability).

3. **Completeness.** The acceptance criteria enumerate the happy path **and** the empty state **and** the error/failure path **and** the disabled/edge state — not just the happy path. A happy-path-only criteria list is a contract violation (satisfies `milestone-driver/agents/triage-reviewer.md` criterion 3, Completeness).
...
- Layer: <the architectural layer the architect assigned this work — OPTIONAL; present only when the candidate carried a `layer` field; cites the stated architecture (.project/<doc>#<section>, a sibling path (anchor), or a sibling file:line) that places it; the anchor form is defined in milestone-driver/skills/citation-format.md, is preferred when the cited region will outlive the line it sits on today, and is never required, so a file:line ref stays a correct choice, not a legacy one; where the cited region is a heading, the heading ref remains the form to write; any one reference carries an anchor or a line number, never both; omitted entirely when no layer was assigned>
...
- Each `Convention followed:` line, and each pattern-to-mirror reference a **ui** issue records under contract clause 5, cites a REAL project-docs ref, a `path (anchor)` ref, or a `file:line` you verified to exist — grep before you cite. A `Convention followed:` line pointing at an artifact you did not confirm exists is a contract violation, and the anchor form is held to that identical standard: an anchor recorded without grep-verifying it against the live repo is the same violation. The grep this gate already mandates is what supplies the anchor: the literal string you find during that grep is what goes inside the parentheses, picked by you, the citing author, to be unique enough to name the region you mean. **Wrap the whole reference in one code span**, opening before the path and closing after the final `)`; that span is what marks the text a citation, so the same words written bare read as a file name plus a parenthetical aside, resolve to nothing, and warn no one. A path that itself contains ` (` takes neither the anchor form nor a heading form; cite `path:line` or `path:start-end` for that file. The requirements above are stated here so you can apply them; the definitions behind them live in `milestone-driver/skills/citation-format.md` (D1's Parse rule for `path (anchor)` and for the path that itself contains ` (`, D2 for `path:line` and `path:start-end`, D1's `What marks it as a citation` for the position test, and D3 for fail-closed resolution).
