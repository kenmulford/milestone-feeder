...
**Both separator forms are matched, with no expiry.** The rows above read the hyphen form `plan` emits today: the `### #X - <title>` heading carrying each issue's slug, title, and tags, plus the two bracket markers. `create` **also** matches the pre-change form of that heading and of both markers, which carried an **em dash (U+2014)** at each position the hyphen now occupies. The tolerance covers a **surviving** issue's heading exactly as it covers a parked or dropped one, so a plan file approved before the form changed still deploys **whole**, not in part. It never sunsets: it is a parse-token shim over an already-approved, gitignored local plan file, not a config key or a decision the user makes, so it carries no discovery notice of its own. That mirrors the shape of the indefinite fallback recorded at `SPEC.md` ("backward compatible: existing repos keep working"), which also shipped without one. The pre-change separator is **named here rather than reproduced**, because `skills/**` is on the surface the em-dash ban scans; `.project/conventions.md` "Em-dash ban" names its own worked example's separator the same way, for the same reason. `update` inherits this tolerance through its existing citation of this step (`skills/update/SKILL.md` Step 2) and restates none of it.
