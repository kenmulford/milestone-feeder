---
name: create
description: This skill should be used when the user invokes "/milestone-feeder:create <brief>", or asks to "create the milestone", "deploy the approved plan", or "build the milestone and issues". Deploys the plan you approved to GitHub (labels, the milestone, every issue, and the build order) exactly as the plan file recorded it; it does NOT re-check or re-plan.
---
...
Read `.milestone-config/feeder.json`. **Absent → invoke `milestone-feeder:setup`** (it bootstraps the profile, aligns the label taxonomy, and returns control), then continue: the user does not re-run the command (`skills/setup/SKILL.md` Phase 5). `create` reads the same own-keys as `plan` (`skills/plan/SKILL.md` Step 0: `projectDocs`, `architectAgent`, `issueAuthorAgent`, `issueSize`); on the found path it consumes **none** of them (it deploys the recorded plan and regenerates nothing), and on the run-`plan`-first fallback `plan` itself consumes them. The only configuration `create` itself needs is GitHub write access (the `gh` surface in the passes below).
...
**Run `create`'s one-time Step-0 notice.** From the repo root, run the notice emitter once: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/emit-notice.sh" create` on a host with bash, `pwsh -NoProfile -File "${CLAUDE_PLUGIN_ROOT}/scripts/emit-notice.ps1" create` on a Windows host without one. It walks `scripts/emit-notice.json` **in file order**, selects every unit whose `skills` list names `create`, and for each performs the marker gate, the trigger check, the print, and the marker write in one step. Never re-type a notice as free-form agent text. What each unit says, when it fires, and why is recorded at `docs/one-time-notices.md (How each skill runs this file)`. Exactly one unit currently names `create`: the **md-epic parent notice**, whose marker is **shared with `update`'s Step 0**, so it shows at most once per clone across both verbs. The emitter is **best-effort** and never aborts the `create` run; read-only except the `.runtime/` dir and the marker. A bash host without `jq` emits nothing; a missing script, an unusable data file, or a malformed unit is **skipped for that entry only**.
...
| **Per surviving issue**: slug, title, the FULL §4 `ISSUE_BODY` verbatim, labels (`ui`\|`logic` + `risk:*`), surface/risk | The issues to create: read verbatim, **no regeneration** (pass c). |
| **Parked issues**: read from the **bracketed marker on the `### #X - <title>` heading** (`[parked - needs product input]`, `docs/plan-file-contract.md` Plan-file output template), not a discrete `kind:` field | Marked in the plan file; routed to the needs-input report (pass e). |
| **Dropped issues**: read from the **bracketed marker on the `### #X - <title>` heading** (`[dropped - depends on parked #X]`, `docs/plan-file-contract.md` Plan-file output template) | Marked in the plan file; routed to no pass (a dependent of a parked issue can't build). |
...
**Both separator forms are matched, with no expiry.** The rows above read the hyphen form `plan` emits today: the `### #X - <title>` heading carrying each issue's slug, title, and tags, plus the two bracket markers. `create` **also** matches the pre-change form of that heading and of both markers, which carried an **em dash (U+2014)** at each position the hyphen now occupies. The tolerance covers a **surviving** issue's heading exactly as it covers a parked or dropped one. It carries no discovery notice. The pre-change separator is **named here rather than reproduced**, because `skills/**` is on the surface the em-dash ban scans. `update` inherits this tolerance through its existing citation of this step (`skills/update/SKILL.md` Step 2) and restates none of it.
...
Deploy the plan file's recorded issue set to GitHub in this **fixed pass order** (the v0.2.0 apply write-sequence, preserved verbatim; only the *source* of the bodies / labels / waves / title changed: `create` reads them from the plan file, regenerating nothing; `docs/specs/v0.3.0-humanize-the-surface.md` §3, §4). Passes **a–d** make those writes through the script twin `scripts/deploy-write-sequence.sh` / `.ps1` (its entry point named per pass below); the judgment stays here. Full mechanics for passes **a–d** (the invocation, the job-file shape, the create-or-adopt table, the deploy-receipt back-write, the slug→`#n` map, and the substring-safe rewrite rule) live in **`docs/create-deploy-sequence.md` → "Step 3: deploy write-sequence (passes a-d)"**, each entry point's own contract in the twin's header:

| Pass | What it does |
|---|---|
| **a. Ensure labels** (`labels`) | Upsert the four canonical labels (`gh label create … --force`) **FIRST**, so every later `--label` resolves; re-runs never duplicate. |
| **b. Create-or-adopt the milestone (by EXACT title)** (`find-milestone`, then `create-milestone` / `reopen-milestone`, then `write-receipt`) | Resolve by the exact title: create / adopt (reopen if closed). The search returns **every** match and picks none: these rows decide, and log the multiple-match notice. Then write the deploy receipt (`Milestone number (GitHub): <n>`) back to the plan file (quote-safe `env.t` resolve; the back-write **reports, never blocks**). |
| **c. Create each surviving issue; build the slug→`#n` map** (`create-issues`) | Create **only** the non-parked / non-dropped issues; on adopt, reuse by exact title (no duplicates), adopted bodies left as-is; apply each issue's recorded labels. |
| **d. Second pass: rewrite slug→`#n`** (`apply-bodies`, `patch-description`, `rewrite-slugs`) | With the **complete** map, rewrite every slug occurrence (newly-created issue bodies + the milestone description) **substring-safe** (descending slug-length order, token-boundary match), then PATCH the description (REPLACE form; idempotent). |
...
|---|---|---|
| **Pass (c)**: a `gh issue create` fails mid-loop | **Abort pass (d)**: the slug→`#n` map is incomplete; rewriting against it would write dangling references. `create-issues` exits non-zero with the partial map, and every pass-(d) entry point refuses an incomplete map. **Report** what was created + the captured partial map + which candidate failed. | The adopt + match-by-title path (b → c) reuses the already-created issues by title (no duplicates), creates the remaining surviving issues, then runs pass (d) against the now-complete map. |
