...
| **b. Create-or-adopt the milestone (by EXACT title)** (`find-milestone`, then `create-milestone` / `reopen-milestone`, then `write-receipt`) | Resolve by the exact title: create / adopt (reopen if closed) / **never delete**. The search returns **every** match and picks none: these rows decide, and log the multiple-match notice. Then write the deploy receipt (`Milestone number (GitHub): <n>`) back to the plan file (quote-safe `env.t` resolve; the back-write **reports, never blocks**). |
