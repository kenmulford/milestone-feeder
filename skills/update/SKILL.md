...
6. Link the current milestones' issues as sub-issues of the parent. Reuses #246's entire linking pass (steps 1 through 5: the once-per-parent already-linked fetch, the per-milestone Wave-ordered children, the nested-epic refusal, the per-child link plus re-assert, the cap warning, the end-of-pass report) by reference, unchanged, run over the current manifest's milestones every time. Its own already-linked check (step 4a) makes this a no-op for every milestone whose issues were linked on a prior run; a newly added milestone's issues are the only ones this actually links.
7. Write the manifest receipt, only on a genuinely new parent (step 3's create branch). Reuses #245's receipt read-modify-write mechanic by reference, unchanged.
