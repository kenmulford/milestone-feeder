...
**The pass, in order** (full mechanics, every `gh` form, both shells, live in `docs/update-reconcile-parent.md`):
...
3. Resolve or create the parent by the manifest's `Parent issue (GitHub): #<n>` receipt (then a title match, then create). Reuses that twin pair's `resolve-or-create` entry point by reference, unchanged, and gated on the receipt: when the manifest already carries one, adopt that number directly and call nothing here, leaving the body write to step 4's diff-gate (that entry point REPLACE-PATCHes on its adopt branch, which is right for a first-time deploy and would double-write on a reconcile); invoke it only when the receipt is absent. Unlike a milestone, which `update` never creates (Step 3b), the parent issue IS create-or-adopt, matching `create`'s own contract for it; this is the one write `update` newly gains the ability to originate.
4. Diff-gate the body write (the fix this issue's triage advisory called for). When the parent already existed before this run (a receipt was already on the manifest), fetch its live body first and compare it to step 2's freshly rendered body. Identical: write nothing, no PATCH; this is what makes the idempotent-re-run acceptance criterion actually hold, since `create`'s own pass PATCHes unconditionally every run, which is correct for a first-time deploy but wrong for a reconcile. Differ: PATCH with the same REPLACE-form rewrite that pass applies on adopt, reused by reference. On a genuinely new parent (no prior receipt), there is nothing to diff against; the create/adopt in step 3 already wrote the correct body, so no separate PATCH follows.
5. Detect a removed milestone. From the same live body fetched in step 4, read its OLD `number:` lines (the block as it stood before this run) and compare them to step 1's freshly gathered set. Any number present in the OLD set but absent from the new one was dropped by the re-plan: its `md-epic-order` entry is simply not re-emitted (step 2 already renders from the current manifest only), its sub-issue links to the parent are left in place, never unlinked, it is never closed or deleted, and it is flagged in this run's report, mirroring `update`'s existing "On GitHub, NOT in plan" convention (Step 4's reconcile table) for a live object no longer in the plan.
6. Link the current milestones' issues as sub-issues of the parent. Reuses that twin pair's `link-sub-issues` entry point, the entire linking pass (the once-per-parent already-linked fetch, the per-milestone Wave-ordered children, the nested-epic refusal, the per-child link plus re-assert, the cap warning, the end-of-pass report), by reference, unchanged, run over the current manifest's milestones every time. Its own already-linked check makes this a no-op for every milestone whose issues were linked on a prior run; a newly added milestone's issues are the only ones this actually links. The rows it prints are turned into notices and the report by the same row table `create` follows.
7. Write the manifest receipt, only on a genuinely new parent (step 3's create branch). Reuses that twin pair's `write-receipt` entry point by reference, unchanged.

**Failure semantics.** Inherited unchanged from #245 and #246: a `gh` error at any step stops that step, reports what completed and what remains, and deletes nothing; a re-run resumes via the same receipt/adopt-by-title/already-linked-check resolve. No new resume machinery.
