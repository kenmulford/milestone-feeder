...
**Run `update`'s one-time Step-0 notices by iterating the shared reference, not by restating their procedural prose.** `docs/one-time-notices.md` is the single canonical source for seven notices/self-heals shared across `plan`, `create`, and `update` (see that file's "Section fields" and "How each skill runs this file"). Iterate its `##` sections **in file order** and, for each section whose **Skills** field includes `update`, run that section's recorded **deterministic emitter twin** (the `bash` form or the PowerShell 7+ form) **verbatim**: the emitter performs the trigger check, prints the section's **Text** when triggered, and writes the section's **Marker**, all in one step. Never re-type a notice as free-form agent text. Two sections currently match `update`: the **Implied-surfaces notice** (§5, whose Marker is **shared with `plan`'s Step 0**) and the **md-epic parent notice** (§6, whose Marker is **shared with `create`'s Step 0**). Each shows at most once per clone across its shared verbs. Every emitter is **best-effort**: it swallows any failure (unwritable dir, permission error, missing `jq`) and never aborts the `update` run; read-only except the `.runtime/` dir + marker (the Implied-surfaces emitter never writes the overlay). A malformed or missing section is **skipped for that entry only**.
