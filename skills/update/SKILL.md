...
4. Diff-gate the body write. When the preliminary read returned a number, invoke the new twin pair's `diff-gate` entry point over the parent, step 2's rendered body file, and a path for it to save the live body to: it fetches and saves that live body, compares, and PATCHes only when the two differ, printing the `compare` and `patch` rows this step reports from. This is what makes the idempotent-re-run acceptance criterion actually hold, since `create`'s own pass PATCHes unconditionally every run. On a genuinely new parent (the preliminary read came back empty), do not invoke it: there is nothing to diff against, and the create/adopt in step 3 already wrote the correct body (`docs/update-reconcile-parent.md` "Step 4: diff-gate the body write").
5. Detect a removed milestone. When the preliminary read returned a number, invoke that twin pair's `detect-removed` entry point over the live body step 4 saved plus step 1's freshly gathered numbers: it prints one row per number the re-plan dropped. Each dropped milestone's `md-epic-order` entry is simply not re-emitted (step 2 already renders from the current manifest only), its sub-issue links to the parent are left in place, never unlinked, it is never closed or deleted, and it is flagged in this run's report, mirroring `update`'s existing "On GitHub, NOT in plan" convention (Step 4's reconcile table) for a live object no longer in the plan. On a brand-new parent, do not invoke it: there is no prior state, so nothing can have been removed (`docs/update-reconcile-parent.md` "Step 5: detect a removed milestone (new logic)").
...
| **Roadmap parent, every current milestone's issues already linked as sub-issues** (Step 1R, step 6) | **NO-OP for linking**: #246's own already-linked check (reused by reference) skips every child; nothing double-links. |
...
- **The roadmap's md-epic parent is reconciled on a re-plan, gated on a roadmap manifest (Step 1R).** When `.milestone-feeder/roadmap-<slug>.md` exists for the brief, `update` also reconciles the shared parent issue `create` produces on the roadmap path: it re-renders the `md-epic-order` block to the current build order, resolves or creates the parent by the manifest's `Parent issue (GitHub): #<n>` receipt (the one object `update` newly gains the ability to create; a milestone itself is still never created, Step 3b), links each milestone's issues to it as sub-issues, and flags (never unlinks, never closes, never deletes) any milestone the re-plan dropped. The parent-body PATCH is diff-gated against the live body: unchanged means zero writes, so an idempotent re-run performs no additional parent-reconcile write. Absent manifest: this step never runs, and the single-plan path stays byte-unchanged. Reuses #245's and #246's mechanics **by reference**; authors no second definition of the block-render, the parent create-or-adopt, or the sub-issue-linking logic (`docs/update-reconcile-parent.md`).
