...
- The **candidate** — its tag, title, surface/risk hint, sketch, **the architect's optional `disposition: grounded | implied`**, **and the architect's optional `layer`** (all from Step 3). Enumerate `disposition` alongside the other fields so the issue-author **receives it at runtime** — the field `agents/issue-author.md` → "What you receive" consumes (made aware by #182). **Default/omitted = `grounded`**: a candidate the architect did not mark `implied` carries **no** `disposition` field in the brief. This thread is **load-bearing** — it closes the architect→plan→issue-author chain (#179 emits the field on `CANDIDATES`, #182 reads it on the author side); without handing it in here the author never receives it and the implied-surfaces feature stays inert.
...
- The **candidate's file-map** — **built HERE at this dispatch**, scoped to THIS candidate's NEIGHBORHOOD (its sketch `path:line` / `path (anchor)` seeds + the architect edges touching it → whole-folder expansion → keyword fallback only when it cites no seed of either form), the ordered `{ path, anchors }` pointer list — a supplement, not an allowlist, degrading like the digest. **Built as recorded in `docs/file-map.md`** — per-candidate, not resolved once at Step 0.
- The **resolved consumer issue-template** from Step 0 — the resolved template content, not a path to walk, so the author writes to the consumer repo's own issue structure. It **supplements, never replaces** the author's Read/grep path and **never lowers the Rigor gate**; an **empty resolution is not an error** — the author uses its built-in §4 default (`docs/step-0-grounding.md` §5).
...
This step writes a single reviewable plan file over the **surviving issue set** Step 5's drop pass already produced; parked and dropped candidates are carried as markers only, never as buildable issues. **No GitHub writes occur — `plan` writes only local scratch files.**
...
The plan file and the report are local scratch (`## Non-negotiables`, the "writes NO GitHub state" bullet, the canonical statement of what `plan` does not write).
