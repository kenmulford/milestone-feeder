...
**Run `plan`'s one-time Step-0 units.** From the repo root, run the notice emitter once: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/emit-notice.sh" plan` on a host with bash, `pwsh -NoProfile -File "${CLAUDE_PLUGIN_ROOT}/scripts/emit-notice.ps1" plan` on a Windows host without one. It walks `scripts/emit-notice.json` **in file order**, selects every unit whose `skills` list names `plan`, and for each performs the marker gate, the trigger check, the print, and the marker write in one step. Never re-type a notice as free-form agent text. What each unit says, when it fires, and why is recorded at `docs/one-time-notices.md (How each skill runs this file)`, the human-readable reference; the script and its data file are the runtime. Every unit is **best-effort and never aborts `plan`**, and read-only except for the `.runtime/` dir + marker and the self-heal, which writes the nested `.gitignore`. A bash host without `jq` emits nothing; a missing script, an unusable data file, or a malformed unit is **skipped for that entry only**.
