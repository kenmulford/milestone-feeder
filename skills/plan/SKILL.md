...
| **File-map** | The ordered, `sourceGlobs`-bounded `{ path, anchors }` discovery index — shape, deterministic ordering, Markdown-only anchor extraction + non-Markdown handling, degrade matrix, never-persisted. |
...
Step 0 only **produces** these; the Step-3 architect brief and Step-4 issue-author brief that **consume** them are wired at Steps 3 / 4. When `uiSurfaceGlobs` degradation applies (below), the digest and file-map are unaffected — they carry no UI-surface distinction.
...
- The **resolved file-map** from Step 0 — the ordered, `sourceGlobs`-bounded `{ path, anchors }` index — handed to the author beside the digest; a supplement, not an allowlist, degrading like the digest. Resolved as recorded in `docs/step-0-grounding.md`.
