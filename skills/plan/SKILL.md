...
name: plan
description: >-
  This skill should be used when the user invokes "/milestone-feeder:plan <brief>", or asks to "plan a milestone from a brief" or "turn an idea into a reviewable plan". Turns a feature brief into a reviewable local plan file of small, well-formed candidate issues you review before `create` deploys them.
---
...
- **The single-brief path is the default and is unchanged: it invokes the inner routine exactly ONCE**, with the whole brief as the lone brief-slice and its sole build-order position, and nothing about the emitted plan file changes on that `none`-signal path. The oversized-brief **roadmap route** is added separately at the front-door, Step 3.6, acts ONLY when the architect raises the multi-milestone signal, and ships its **own** one-time discovery notice (Step 0) per `SPEC.md` §3.1.
...
**Run `plan`'s one-time Step-0 units by iterating the shared reference, not by restating each notice's procedural prose.** `docs/one-time-notices.md` is the single canonical source for seven notices/self-heals shared across `plan`, `create`, and `update` (see that file's "Section fields" and "How each skill runs this file"). Iterate its `##` sections **in file order** and, for each section whose **Skills** field includes `plan` (sections 1–5 **and 7** (`docs/one-time-notices.md` §Contents: the self-heal plus four printed notices, the last of which (Implied-surfaces) is shared with `update`, plus §7's consumer issue-template notice)), run that section's recorded **deterministic emitter twin** (the `bash` form or the PowerShell 7+ form) **verbatim**: the emitter performs the trigger check, prints the section's **Text** when triggered, and writes the section's **Marker**, all in one step. Never re-type a notice as free-form agent text. Section 6 (md-epic parent) is the only section that does not list `plan` in its Skills field, and is never evaluated here. Every unit is **best-effort and never aborts `plan`**, and read-only except for the `.runtime/` dir + marker and the self-heal, which writes the nested `.gitignore`. A malformed or missing section is **skipped for that entry only**: never a crash, never a partial print, never an aborted run.
...
Read the project docs under `projectDocs` **best-effort** (`SPEC.md` §6 Step 0). Honor the `.project/` contract: a section that is absent or marked `[TBD]` is treated as **not present**. It is **skipped, never grounded on**. Degradation follows the shared Step-0 contract below (`docs/step-0-grounding.md (The shared contract: resolve-once / hand-in / degrade / supplement)`). Cite a project-docs-grounded decision as `.project/<doc>.md#<section>` when carrying it forward.

While reading, **also assemble the four Step-0 grounding inputs** the inner routine consumes: the **grounding digest**, the **global implied-surfaces reference**, the **project-local overlay** merged into it, and the **consumer issue-template**. All four obey **one shared contract**: each is **resolved ONCE here** at this OUTER boundary (never re-resolved per candidate / per invocation), **handed unchanged into the inner routine** via the `resolved` bundle, **degrades best-effort: an absent / unreadable / malformed source → an empty / minimal pass-through, no error, no abort** (`.project/design-philosophy.md#Error & failure philosophy`), and **supplements, never replaces** each agent's own mandatory Read/grep rigor gate (no input is an allowlist). Resolve each **exactly as recorded in `docs/step-0-grounding.md`**, which states that shared contract once and records each input's specifics:

| Input | What it is (specifics owned by `docs/step-0-grounding.md`) |
|---|---|
| **Grounding digest** | The compact, ordered set of `.project/<doc>.md#<section>` slices: same existing-sections filter (absent / `[TBD]` skipped, no new rule), a superset that supplements. |
| **Global implied-surfaces reference** | The plugin-bundled `${CLAUDE_PLUGIN_ROOT}/docs/implied-surfaces.md` reasoning prompt the architect's clause-8 consult reads. |
...
The routine body is Steps **1, 2, 3, 3.5, 4, 5, 7** (exactly as written below). **Steps 3.6 (front-door routing) and 3.7 (the roadmap fan-out) are OUTER, run-level orchestration. They are NOT part of this per-milestone routine** (like Step 0): a dispatched per-milestone inner routine runs Steps 1 → 2 → 3 → 3.5 → 4 → 5 → 7 and **never re-enters Step 3.6 or Step 3.7**, so the fan-out cannot recurse into itself. (Step 5.1's ADOPT branch (`docs/version-ladder.md`) and Step 7's `assignedSlug` branch are the optional-parameter seams the fan-out drives; they live **inside** the routine.) The body is otherwise unchanged by this refactor.
...
Record `epicIssueNumber` when the brief was an epic issue. `plan` posts **nothing** to the epic (`## Non-negotiables`, the no-GitHub-state bullet). The number is recorded in the plan file's source-brief reference so the downstream `create` / `update` can route their needs-input report to the epic comment.
...
**It returns** its structured return block (the `CANDIDATES / EDGES / WAVES / PRODUCT_GAPS / SCOPE_SPANS_MULTIPLE_MILESTONES / INVARIANTS` schema) defined verbatim at `agents/architect.md` → "Structured return block" (the owning contract; not restated here). The fields the orchestrator branches on downstream (`disposition`, `blocks`, `SCOPE_SPANS_MULTIPLE_MILESTONES`, and `INVARIANTS`) are kept legible inline just below.
...
**Capture each candidate's optional `disposition`** (`grounded` | `implied`) verbatim alongside its `tag` / `title` / `surface` / `risk` / `sketch`. **Default/omitted = `grounded`** (`agents/architect.md` → the optional `disposition` field). It is **additive**: a candidate the architect did not mark `implied` carries no `disposition`. `plan` threads this field into the Step-4 issue-author brief and renders `implied` candidates distinctly at Step 7 (below); it invents no disposition and overrides none.

**Capture each candidate's optional `layer`** (the architectural layer the architect assigned, `agents/architect.md` clause 9) verbatim alongside its `tag` / `title` / `surface` / `risk` / `sketch` / `disposition`. It is **additive** in exactly the same way as `disposition`: a candidate the architect did **not** assign a layer carries **no** `layer` field. `plan` threads this field into the Step-4 issue-author brief (the author records it in the issue's Design block, so the driver sees the layer); the layer's *ordering* rides the architect's `EDGES` unchanged. A layer edge is an ordinary edge (`agents/architect.md` clause 9), consumed by the Step 5 graph like any other. `plan` invents no layer and overrides none.

**Capture `SCOPE_SPANS_MULTIPLE_MILESTONES`** exactly parallel to how `PRODUCT_GAPS` is consumed: record it **verbatim** when raised (the list of `{ milestone, tags }`, the architect's proposed split); the literal `none` when absent. `plan` does **NOT** re-partition. It carries the architect's proposed split verbatim (the architect owns the structural read; `plan` only surfaces it). **Non-blocking invariant (scoped to the front-door's path).** This signal NEVER **aborts** the run and is NEVER silently dropped. On the **`none` path**, and on the **oversized-but-degraded** path where the front-door's route falls back (Step 3.6), it does not gate: `plan` produces today's deployable **single-milestone** plan, unchanged, and never alters `CANDIDATES` / `EDGES` / `WAVES` / the surviving issue set / Wave order / milestone identity. On the **oversized** path, the front-door (Step 3.6) **routes** the brief into `build-roadmap` *before* the Step 4 fan-out. The route is itself a non-blocking, surface-for-confirm checkpoint that **supersedes** the single-milestone pipeline for this brief (a decline or failure degrades back to it); it never hard-blocks. **Edge:** because the signal is sourced from the architect's structural read of `CANDIDATES` (not the surviving issue set), it is carried verbatim and surfaced **even if every candidate is later parked/dropped at Step 3.5 or the Step 5 drop pass**. It is independent of which issues survive. Grounding: `docs/specs/v0.3.1-driver-handoff.md` §5 (detection + advisory, non-blocking) and §6 (the additive Multi-milestone advisory plan-file field).

**Capture `INVARIANTS`** (`agents/architect.md` clause 10) verbatim: each entry's `key`, `value`, `citation`, and `applies_to`. `INVARIANTS` is the literal `none` when no directive binds two or more candidates, the same always-emitted sentinel shape `PRODUCT_GAPS` carries. `plan` threads each entry into the Step-4 brief of every candidate its `applies_to` names (below). It invents no invariant, rewords no `value`, and overrides none. `applies_to` is keyed to `CANDIDATES` **as returned**, so an entry that also names a candidate pre-parked at Step 3.5 still binds the surviving candidates it names.
...
3. **This REUSES the product-gap park routing and the drop-parked-issues + drop-dependents rule, applied earlier. It adds no new park mechanism.** A pre-parked candidate lands in `productGaps[]` and is routed exactly as any product-gap park (the same `needs-product-input` record). Because a Step-3.5 pre-park is a product-gap entry in `productGaps[]` (exactly the input Step 5's drop pass already consumes), the **Step 5 drop pass** (the "drop parked issues + drop dependents + re-render" step at the head of Step 5) removes the pre-parked candidate **ITSELF** (not only its dependents) from the rendered milestone description, so the Wave order never leaves a phantom tag naming a parked candidate. The drop-**dependents** rule applies on top: a candidate that **transitively depends** on a pre-parked candidate is dropped (a dependent of a parked issue cannot build), removed from the Step 4 set, and marked dropped-because-dependency. The only thing Step 3.5 changes is **WHEN** the park happens (before the fan-out vs during it), for the gaps the architect already linked to candidates. **The Step 4 join + Step 5 drop pass remains the catch-all**: a gap the architect did **NOT** link to candidates (one an issue-author surfaces during the fan-out as `STATUS: PRODUCT_GAP`) parks there instead, unchanged.

### Step 3.6: Front-door size/scope check: route an oversized brief into `build-roadmap`

**Boundary: Step 3.6 is OUTER, run-level orchestration. It is NOT part of the per-milestone inner routine** (like Step 0). It runs once per run on the main thread; a dispatched per-milestone inner routine (Steps 1, 2, 3, 3.5, 4, 5, 7) does **not** re-enter it.

This is the **front-door** size/scope check. It runs after the architect's structural read is fully captured (Step 3) and after the Step-3.5 pre-park, and **before the Step 4 issue-author fan-out**. The architect's `SCOPE_SPANS_MULTIPLE_MILESTONES` signal is its **SOLE arbiter** of "oversized". The front-door introduces **no second threshold metric of its own**; the borderline call is the architect's, carried verbatim. **Initialize `roadmapRouteTaken = false` HERE, before branching on the signal.** Every path (including `none`, which never enters the route below) then leaves the flag in a defined state for Step 7's read, regardless of which branch runs. Branch on the captured signal:

| Captured signal | Front-door action |
|---|---|
| **`none`** (single coherent release) | **Do not route.** Fall straight through to Step 4 on the whole brief (the single-milestone pipeline, **unchanged**). No `build-roadmap` invocation, no advisory change, no new prompt. |
| **Raised** (≥2 milestones forming a strict partition of `CANDIDATES`) | **Route into `build-roadmap`** (the internal skill at `skills/build-roadmap/SKILL.md`). See below. |
...
`build-roadmap` dispatches the roadmap-splitter once, **surfaces the proposed split for the user to confirm / merge / split / reorder / reject** (it owns that checkpoint; `plan` does not re-surface it), and returns control with **either a confirmed manifest path or none** (`skills/build-roadmap/SKILL.md` Step 4). The hand-off's **manifest shape is OWNED by `build-roadmap`** (`skills/build-roadmap/SKILL.md` "Manifest format"). `plan` forwards the manifest path; it does **not** redefine its shape. Consume the return:

| `build-roadmap` return | Front-door outcome |
|---|---|
| **A confirmed manifest path** | **Route taken.** Surface the confirmed roadmap + the manifest path, and hand it to the **per-milestone planning fan-out (Step 3.7)**. The fan-out runs the single-milestone inner routine (Steps 1–7) once per milestone the roadmap names, in build order. (**Step 3.6** owns detect + route + invoke + surface + hand off; **Step 3.7** owns the fan-out itself.) On this path the single-milestone **Steps 4–7 do NOT run for the whole brief** (the fan-out plans each milestone instead), so the Step-7 passive *"~N milestones"* advisory is **SUPERSEDED**. The confirmed roadmap was surfaced in its place. |
...
**Boundary: Step 3.7 is OUTER, run-level orchestration. It is NOT part of the per-milestone inner routine** (like Step 0 and Step 3.6). It runs once per run on the main thread and **dispatches** the inner routine per milestone; a dispatched inner routine runs Steps 1, 2, 3, 3.5, 4, 5, 7 and **never re-enters Step 3.6 or Step 3.7** (no recursion). Its only seams INTO the routine are the two optional parameters (`preResolvedVersion`, `assignedSlug`) consumed by Step 5.1's (`docs/version-ladder.md`) and Step 7's ADOPT branches.

This is the **roadmap branch**. It runs **ONLY** when Step 3.6 set `roadmapRouteTaken` = **true** (a confirmed manifest path). On **every other** Step-3.6 outcome (signal `none`, degrade, decline, single-milestone-after-all) `roadmapRouteTaken` is **false**, this step is **SKIPPED**, and control has already fallen through to **Step 4** on the whole brief (the single-milestone pipeline **unchanged**). **The fan-out lives only here; the normal-sized-brief path never reaches it** (acceptance criterion: disabled/edge state, single-milestone planning is unchanged).
...
**Consume the confirmed manifest (re-derive nothing).** Read the manifest at the path `build-roadmap` returned (Step 3.6): its milestone entries (name, brief slice, `Build-order position` 1..N) and its `Build order:` line. **The manifest shape + the cross-milestone build order are OWNED by `build-roadmap`** (`skills/build-roadmap/SKILL.md` "Manifest format"); the fan-out **consumes** them and **never re-partitions or re-derives the order** (`docs/specs/v0.3.1-driver-handoff.md` §6; `.project/design-philosophy.md#Layering & boundaries`: skills orchestrate, the manifest is the build artifact, `plan` does not redefine its schema).
...
- The **candidate**: its tag, title, surface/risk hint, sketch, **the architect's optional `disposition: grounded | implied`**, **and the architect's optional `layer`** (all from Step 3). Enumerate `disposition` alongside the other fields so the issue-author **receives it at runtime**: the field `agents/issue-author.md` → "What you receive" consumes. **Default/omitted = `grounded`**: a candidate the architect did not mark `implied` carries **no** `disposition` field in the brief. This thread is **load-bearing**. It closes the architect→plan→issue-author chain (`agents/architect.md` clause 8 emits the field on `CANDIDATES`; `agents/issue-author.md` → "What you receive" reads it on the author side); without handing it in here the author never receives it and the implied-surfaces feature stays inert.
  - **Enumerate `layer` the same way**: hand the candidate's `layer` (the architectural layer, `agents/architect.md` clause 9) to the issue-author so it **records the layer in the issue's Design block** (`agents/issue-author.md` → "What you receive"), and the driver sees which layer the work sits in. **Additive**: a candidate the architect assigned **no** layer carries **no** `layer` field in the brief. This thread is load-bearing the same way `disposition` is. Without handing it in here the author never receives it and the layer never reaches the issue body.
- The **invariants binding this candidate** (`agents/architect.md` clause 10): the `INVARIANTS` entries captured at Step 3 whose `applies_to` names this candidate's tag. Hand each scoped entry in whole: its `key`, `value`, `citation`, and `applies_to`. The author transcribes every handed-in `value` **verbatim** and carries its `citation` as the grounding ref (`agents/issue-author.md` → "What you receive"), so `plan` passes the text through byte for byte and rewords nothing. `applies_to` rides along as scoping context only. The author records the `value` and its `citation`, never the sibling tags, which are not edges and never become a `Depends on` line. **Additive, and explicitly empty rather than absent**: an empty list is handed to a candidate that no entry's `applies_to` names, and to every candidate when `INVARIANTS` is the literal `none`. The author then transcribes none. This thread is load-bearing the same way `disposition` and `layer` are. Without handing it in here the author never receives a `value` to transcribe, and no invariant reaches an issue body.
- The **brief** (the grounding source carrying *what to build*, in product terms) **and the resolved grounding digest** from Step 0 (the `.project/<doc>.md#<section>` slices assembled there) as the author's project-docs grounding. It is handed in under the shared Step-0 contract (`docs/step-0-grounding.md (The shared contract: resolve-once / hand-in / degrade / supplement)`): that contract governs how an empty digest degrades and how the digest supplements, never replaces, the author's `agents/issue-author.md` Rigor gate.
  - **The digest also surfaces which `.project` config docs EXIST**: the docs the author POINTS each issue at, so the driver's triage + implementer read the source of truth at build time. Brief the author to add a **config-pointer** line to the issue's existing `## Design` block keyed to what the issue touches: styling → `.project/tokens.json` + `.project/design-system.md#<section>`; deployment/env → `.project/environment.md` (a touched convention already rides the `Convention followed:` line). The pointer is a **reference to the PATH only, resolved values NOT inlined** (no hex, no parsed token values, no pre-solved render; the render/tokens are the driver's to consume: `agents/issue-author.md` → "Point each issue at the project's config"). **Additive**: an issue touching none of these, or a project genuinely missing the doc (grep/confirm the doc exists before pointing, exactly as the author's Rigor gate requires for any citation; **not** keyed on digest-presence: the digest carries markdown `.project` slices only, so a JSON config doc like `.project/tokens.json` is confirmed by grep/existence and is **never** in the digest), carries **no** pointer: no error, no fabricated reference.
- The **candidate's file-map**: **built HERE at this dispatch**, scoped to THIS candidate's NEIGHBORHOOD (its sketch `path:line` / `path (anchor)` seeds + the architect edges touching it → whole-folder expansion → keyword fallback only when it cites no seed of either form), the ordered `{ path, anchors }` pointer list. It is a supplement, not an allowlist, degrading like the digest. **Built as recorded in `docs/file-map.md`** (per-candidate, not resolved once at Step 0).
- The **resolved consumer issue-template** from Step 0: the resolved template content, not a path to walk, so the author writes to the consumer repo's own issue structure. Handed in under the shared Step-0 contract (`docs/step-0-grounding.md (The shared contract: resolve-once / hand-in / degrade / supplement)`); on an empty resolution the author uses its built-in §4 default (`docs/step-0-grounding.md` §5).
...
**Drop parked issues + their dependents FIRST (the drop pass).** Before building the graph, compute the **surviving issue set** = every authored candidate **MINUS** every parked candidate **MINUS** every candidate that transitively depends on a parked one. Parked candidates are the **product-gap** parks gathered so far: the Step-3.5 pre-parks, plus any Step-4 `STATUS: PRODUCT_GAP` return (routed to `productGaps[]` at the Step-4 join). **Remove each parked candidate ITSELF** from the milestone. It is routed to the needs-product-input report (Step 7), never rendered as a buildable issue. Then **drop every issue that transitively depends on a parked one** (a dependent of a parked issue cannot build), marking it dropped-because-dependency. Render the graph + milestone description over the **surviving set only**, so the Wave order never leaves a phantom tag naming a parked or dropped candidate. This is the drop-parked-issues + drop-dependents + re-render pass Step 3.5 already invokes for its pre-parks (`skills/plan/SKILL.md` Step 3.5 clause 3).
...
The plan file and the report are local scratch (`## Non-negotiables`, the no-GitHub-state bullet, the canonical statement of what `plan` does not write).
