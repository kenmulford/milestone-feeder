...
**Per-dispatch verify + one bounded retry (before the `STATUS` routing rule below).** Before reading a returned wrapper's `STATUS`, verify it against `agents/issue-author.md` → "Output format". **Malformed** = a field the declared `STATUS` requires is missing (`AUTHORED` requires `ISSUE_TAG`, `TITLE`, `ISSUE_BODY`, `LABELS`; `PRODUCT_GAP` requires `ISSUE_TAG`, `PRODUCT_GAP`); **or** a required field is present but VACUOUS: an `AUTHORED` return whose `ISSUE_BODY` has no sections at all (whichever structure it was authored to: the §4 default or a resolved consumer template), or whose `LABELS` is empty (`[]`, no `ui|logic` entry); **or** `STATUS` is neither `AUTHORED` nor `PRODUCT_GAP`. **Empty** = no returned text at all, taking the same one retry a malformed return takes. **Also malformed: a `STATUS: AUTHORED` return that dropped or reworded a handed-in invariant.** Every handed-in invariant's `value` must appear in the returned `ISSUE_BODY` as a literal substring, byte for byte as it was handed in; a body missing any one handed-in `value` is malformed, so matching one of several scoped entries does not pass. A candidate handed an **empty list** makes this check a **no-op**. It is a substring comparison over text `plan` already holds: no reviewer dispatch, no post-fan-out reconcile pass.

**On malformed or empty, re-dispatch the SAME issue-author call ONCE** for this candidate (same candidate, brief, digest, file-map, issue-template, invariants, edges, shared keys, scoped `productGaps[]`), **feeding back which check failed**: e.g. "your prior return had no `ISSUE_BODY`", and on an invariant miss the `key` **and** the exact `value` of every entry that failed to match. Both the dispatch and its one retry are **same-run, immediate re-dispatches of the identical agent: never a queued or deferred job** (`.project/environment.md#Async & messaging`). The retry re-uses this candidate's own slot in the cap-4 rolling window above, leaving the window's in-flight accounting unchanged. If it now parses, read its `STATUS` and classify exactly as a first-attempt success would (the routing rule just below).

**The bound: if the retry (the SECOND consecutive attempt for this candidate) ALSO returns malformed or empty, make NO third attempt, full stop.** Route the candidate into `productGaps[]` via the **same** mechanism and wrapper shape a genuine `STATUS: PRODUCT_GAP` return already takes (below): **`ISSUE_TAG: <tag>` as its own structured field** (what Step 5's drop pass keys on), never buried in free text, plus a synthetic `PRODUCT_GAP: { what: "issue-author dispatch failed verify twice for <tag> (malformed/empty return)", why: "<which checks failed on both attempts>" }`, whose free text is what reads distinctly from a genuine product-gap park in the needs-product-input report. **No new park mechanism**: it flows through the identical Step-5 drop pass and report, this fan-out's instance of `.project/design-philosophy.md#Error & failure philosophy` ("best-effort... never stop plan").
