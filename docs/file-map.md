...
Unlike the four Step-0 grounding inputs (the grounding digest, the global implied-surfaces reference, the project-local overlay, and the consumer issue-template, all resolved **once** at the Step-0 OUTER boundary and recorded in `docs/step-0-grounding.md`), the file-map is **built PER CANDIDATE at the Step-4 dispatch**, scoped to that candidate's own cited grounding rather than the whole `sourceGlobs` set. Everything else (pointers-only, best-effort degrade, supplement-not-allowlist, never-persisted) it shares with those inputs.
...
1. **SEED.** The seed files are those named by the candidate's cited references in its `sketch` (`agents/architect.md (sketch: <one or two lines)`), plus the files referenced by the architect `EDGES` touching that candidate (`agents/architect.md (Every dependency edge cites the)`). **Both citation forms seed: `path:line` and `path (anchor)`.** The seed is the `path`, extracted per `milestone-driver/skills/citation-format.md` (D1's Parse rule for `path (anchor)`, D2 for `path:line`, and D1's span / position tests for what counts as a citation at all); those rules are authoritative there and are not restated here. The `path` is all the map takes, so no anchor is ever resolved: a stale anchor still seeds its file's folder, and a `path` absent from disk is a seed miss (§5). Each seed pulls in its containing folder (rule 2). A seed's own folder is **never dropped by the cap**: it is the highest-signal input.
...
1. **SEED folders first**, in **citation order** (the sketch refs first, in their order of mention, then the EDGES artifact refs), with **first-mention-wins dedup**. **Within each source, order of mention is the actual order in the text, `path:line` and `path (anchor)` interleaved exactly as written**, never all of one form before the other; first-mention-wins dedup applies across the two forms exactly as it does within one.
...
| A **seed `path`** that does not exist on disk, in either citation form (§2 rule 1) | **Skip that seed** and continue over the rest; it contributes no folder, and the candidate's dispatch is **never aborted**. It is a **seed miss, never a resolver-level failure**: the map extracts the `path` and resolves no anchor, so the fail-closed rule for a missing anchor in `milestone-driver/skills/citation-format.md` (D3, which that file's four-forms table scopes to the `milestone-driver/scripts/resolve-citation.{sh,ps1}` resolver) is never reached here. |
