...
|---|---|
| **Nested-supported** | Dispatch the **inner routine (Steps 1–7)** once per milestone, **EACH in its own background subagent that itself sub-dispatches** architect/issue-author. **Also cap the inner `skills/plan/SKILL.md` Step-4 fan-out at 4** (total ≤16) so compound concurrency stays bounded. |
| **Flat-fallback** | The `plan` **MAIN THREAD** orchestrates each milestone's planning so the architect/issue-author sub-dispatches happen **at the main-thread level**, under the **same cap-4 rolling window** across milestones. **NO subagent spawns sub-subagents.** |
...
The cross-milestone build order's **recorded home is the MANIFEST** (its `Build order:` line + each milestone's `Build-order position`), **owned by the manifest contract** (`skills/build-roadmap/SKILL.md` "Manifest format"). That manifest is the **source of truth** the downstream `create` / driver read for cross-milestone order. The fan-out **records nothing new and writes no build order into the individual per-milestone plan files** (per `skills/plan/SKILL.md` "The single-milestone inner routine (Steps 1–7): callable contract", `buildOrderPosition` does **not** alter the rendered plan file). It only **consumes** the order from the manifest and **surfaces** it to the user in the 3.7.g roster, **never re-deriving it or redefining the schema**; the manifest remains its recorded home (`docs/specs/v0.3.1-driver-handoff.md` §6).
...
**Record each planned milestone's plan-file path into the manifest (close the producer→consumer handle for `create`).** For every milestone classified **planned**, record its real plan-file path into that milestone's entry in the confirmed manifest, in the `Plan file:` field the manifest contract reserves (`skills/build-roadmap/SKILL.md` "Manifest format"). That path is `.milestone-feeder/plan-<assignedSlug>.md`, the `assignedSlug` pre-derived at 3.7.d. Edit the manifest **in place**, consistent with how `plan` writes its own scratch files (`skills/plan/SKILL.md` Step 7). It is local scratch the model already authored, so **no new shell is needed**; key the write to the milestone entry by its `Build-order position`, so re-planning the same roadmap overwrites the field to the **same deterministic path** (idempotent). This records the path into the **MANIFEST only**: it writes **nothing** into the per-milestone plan files (consistent with 3.7.f, which keeps cross-milestone metadata out of the plan files). A **failed-to-plan** milestone's `Plan file:` field is **left pending** (no file exists to point at). When the barrier completes, every planned milestone's entry carries its real plan-file path: the deterministic handle `create`'s deploy-loop resolves each milestone's plan file by (`skills/create/SKILL.md` Step 1R), **never** a slug re-derived from the milestone name.
