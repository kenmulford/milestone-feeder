...
**Enforcement.** Authors hold this ban directly: a gate is a backstop, not the rule. The mechanical check belongs in `scripts/check-vocabulary.sh (PATTERN=)`. Issue #399 owns widening that pattern to fail on a stray em-dash. The ban does not reach `CHANGELOG.md`, `docs/specs/**`, `scripts/**`, or `.github/**`, and #399 adds `tests/**/*observed-*.md` to that set. (Grounded in `docs/plan-file-contract.md (## Plan-file output template)`; `scripts/check-contract-strings.sh (The two contract strings)`; `scripts/check-vocabulary.sh (PATTERN=)`; `agents/architect.md (description)`; `CHANGELOG.md (## v0.13.2: prose compression & contract-string fix)` for the one pinned separator; issue #387.)
