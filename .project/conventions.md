...
**Enforcement.** Authors hold this ban directly: a gate is a backstop, not the rule. The mechanical check lives at `scripts/check-vocabulary.sh (PATTERN=)` and fails on a stray U+2014 anywhere it scans; that script's header, not any spec section, is the check's authoritative definition. The ban itself reaches everywhere, without exception. What the five pathspecs below are excluded from is the SCAN, and they are excluded for two unlike reasons. Which reason applies decides what the exclusion asks of you.

| Excluded from the scan | Why | What it asks of you |
|---|---|---|
| `scripts/**`, `.github/**` | The gate cannot scan them. It has to hold the banned character in order to test for one, so it excludes its own machinery. | Nothing changes: you are as bound here as anywhere. The exclusion is mechanical, never a licence. `scripts/validate-plugin-structure.py` and `.github/workflows/ci.yml` were recast under this ban and carry none. |
