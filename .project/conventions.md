...
**Carve-outs, and only these three.** The first is the plan-file issue heading form and its `[parked ...]` and `[dropped ...]` markers at `docs/plan-file-contract.md (## Plan-file output template)`. The second is the two canonical contract strings pinned at `scripts/check-contract-strings.sh (The two contract strings)`. The third is the five byte-exact quotes in `docs/create-deploy-sequence.md` of sources that will never follow a recast: two from `docs/specs/v0.3.1-driver-handoff.md`, a frozen spec, and three from the `milestone-driver` plugin, a separate repo carrying no such ban. Recasting a quote whose source will never follow turns the quote into a misquote; `scripts/check-vocabulary.sh (ALLOWLIST_ROWS)` allow-lists these five by byte-exact quoted SPAN, never by line and never by file, so a second stray mark elsewhere on an allowlisted line still fails. All three are matched byte-exact rather than read, so whatever separator each carries there is authoritative and is never swept. Changing any of the three is a contract change that re-pins every consumer in the same commit, never a punctuation pass.

**Before recasting a space-hyphen-space, look for a recorded decision.** The v0.13.2 sweep converted em-dashes to hyphens across the repo. Most of those are substitutions this ban requires you to recast, and outside the carve-outs above a bare hyphen never satisfies it. One was pinned as a hyphen on purpose: the issue-author's `Depends on` edge separator, at `CHANGELOG.md (## v0.13.2: prose compression & contract-string fix)`. Neither the mark's shape nor `git log` separates the two, so find the recorded decision before you sweep.

**Enforcement.** Authors hold this ban directly: a gate is a backstop, not the rule. The mechanical check lives at `scripts/check-vocabulary.sh (PATTERN=)` and fails on a stray U+2014 anywhere it scans; that script's header, not any spec section, is the check's authoritative definition. The ban binds what you AUTHOR, everywhere. What the six pathspecs below are excluded from is the SCAN, and they are excluded for two unlike reasons. Which reason applies decides what the exclusion asks of you.

| Excluded from the scan | Why | What it asks of you |
|---|---|---|
| `scripts/check-vocabulary.sh`, `scripts/check-contract-strings.sh`, `.github/**` | Neither gate can scan itself: each has to hold what it tests for. The vocabulary gate holds the retired tokens verbatim, and the contract-strings gate holds the banned character itself, inside the separator class its drift regex matches. `.github/**` rides along and holds none today. The exclusion names two files rather than a `scripts/**` blanket because the REST of `scripts/` is scanned: `scripts/emit-notice.json` carries the lines every one-time notice prints, which is the copy a consumer sees. | Nothing changes: you are as bound here as anywhere. The exclusion is mechanical, never a licence. `.github/workflows/ci.yml` was recast under this ban and carries none. |
| `CHANGELOG.md`, `docs/specs/**`, `tests/**/*observed-*.md` | Recasting them is itself the defect. A shipped changelog entry, a frozen spec, and a scenario run record are evidence of what was written. | Leave them alone. Rewriting one makes it a misquote or falsifies the record. Sweeping them is the error the exclusion exists to prevent. |

(Grounded in `docs/plan-file-contract.md (## Plan-file output template)`; `scripts/check-contract-strings.sh (The two contract strings)`; `scripts/check-vocabulary.sh (PATTERN=)`; `CHANGELOG.md (## v0.13.2: prose compression & contract-string fix)` for the one pinned separator; issue #387.)
