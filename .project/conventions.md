...
(Grounded in `docs/plan-file-contract.md (## Plan-file output template)`; `scripts/check-contract-strings.sh (The two contract strings)`; `scripts/check-vocabulary.sh (PATTERN=)`; `agents/architect.md (description)`; `CHANGELOG.md (## v0.13.2: prose compression & contract-string fix)` for the one pinned separator; issue #387.)
