...
| 3 | Dispatch the **`architect`** agent **once per brief**, plus one incremental dispatch per batch of items added on a re-plan (`docs/incremental-replan.md`): candidate issue set (small, independently-buildable, ~one PR each) + dependency edges + Wave order. |
