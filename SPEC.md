...
- **Scope.** `applies_to` draws its tags from `CANDIDATES` as returned, the full set, never the reduced set the Step 3.5 pre-park and the Step 5 drop pass leave, so an entry naming a candidate parked later still binds the surviving candidates it names.
- **Two or more, never one.** A directive binding exactly one candidate stays that candidate's own design decision, grounded in its sketch under clause 2, never promoted here.
- **Grounded, never invented.** Each entry cites the recorded brief line, the project docs, or an established repo convention, the same rigor gate every design call clears. A directive that cannot be grounded falls back to a clause 2 call on each candidate it would have bound, resolved or parked to `PRODUCT_GAPS`.
- **Still recorded per candidate.** Every tag in `applies_to` still records the directive in its own sketch; the entry pins the one resolution they share.
- A breakdown in which no directive binds two or more candidates returns the literal `none`, and every issue-author receives what it receives today, unchanged. `INVARIANTS:` is an always-required top-level key, and `none` is a valid value for it (`docs/roadmap-fan-out.md` §3.7.e).

**The invariants thread architect → plan → issue-author → issue.** `plan` captures each entry's `key` / `value` / `citation` / `applies_to` verbatim (Step 3), inventing none and rewording none, and at Step 4 hands each candidate the entries whose `applies_to` names its tag, on the same brief list that already carries `disposition` and `layer`. The hand-in is explicitly empty rather than absent: a candidate no entry names, and every candidate when `INVARIANTS` is `none`, is handed an empty list and transcribes nothing. The issue-author transcribes each handed-in `value` into the issue body verbatim, carrying its `citation` as the grounding ref and never re-deriving or overriding it; `applies_to` is scoping context only and never becomes a `Depends on` line. A returned `ISSUE_BODY` missing a handed-in `value` as a literal substring is malformed, caught by `plan`'s existing per-dispatch verify pass, whose one bounded retry feeds back every failing entry's `key` and its exact `value`. A second miss takes the park path any other malformed return takes, no new mechanism. No new config key: the invariants pass reuses the brief and `projectDocs` grounding the architect already reads, so nothing in `feeder.json` / `setup` changes; the discovery path is this spec plus the architect / plan / issue-author contracts.
