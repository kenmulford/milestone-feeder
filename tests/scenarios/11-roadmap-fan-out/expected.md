...
milestone (Step 3.7 fan-out) → (then `create`) **deploy-loop**. This scenario asserts
the three roadmap artifact classes the
...
ordered roadmap of milestones, plans each, and `create` deploys them.**
...
- Each plan file is an **ordinary single-milestone plan** produced by the existing
  ...
  order, the `Milestone title (exact)` line. (Per-milestone
...
- The page-issues (portal / dashboard / report) classify `logic` (uiSurfaceGlobs
  ...
  directive.
- The sandbox create-loop assertions are **silently dropped** rather
