# Expected contract — 05 reviewer backends  (GRADER ONLY · built, run later)

Run the same small brief under two configs.

## A) `reviewer: false`
- MUST: the gate is **skipped**; a visible 🔴 warning is shown to the user; the plan-file status line records `SKIPPED (reviewer:false)`. Issues are emitted unvetted (the warning is the point).
- FAIL if: it silently runs a gate anyway, or omits the warning.

## B) `reviewer: "internal"` (or `"milestone-driver"` with reviewers absent → degrade)
